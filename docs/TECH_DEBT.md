...
| TD-017 | Quick Action buttons on Detail page sidebar are `href="#"` placeholders — never wired | Medium | 2026-05-09 | Wire each action to its target route or modal. Products: Receive stock → `/Receive?sku={code}`, Adjust stock → modal/form (Phase 7+ admin CRUD), Print label → label-print endpoint (TBD). Customers: Create order → `/Orders/Create?customer={code}` (Phase 7+ orders), Send email → already a `mailto:` link, View invoices → `/Invoices?customer={code}` (Phase 7+). Warehouses: Receive shipment → `/Receive?warehouse={code}`, Cycle count → `/CycleCount/Create?warehouse={code}` (Phase 7+), View stock → `/Stock?warehouse={code}` (Phase 7+). Estimate ~30 min per entity once target routes exist; today most targets don't, so the realistic close is a sweep alongside Phase 7+ admin CRUD | Pre-existing from Phase 4 mockup. Surfaced during Phase 6D smoke attempt on 2026-05-09. Functional gap — buttons visible but inert. Workaround: navigate via main sidebar or direct URLs. CustomersController already wires Send email correctly via `mailto:` (the only one that actually works) — the rest are `#` |

...
| TD-015 | `MovementActivityMapper` doesn't resolve actor names or location codes | Low | 2026-05-09 | Add batch lookups: `IUserRepository.GetDisplayNamesByIdsAsync(IEnumerable<Guid>)` + `ILocationRepository.GetCodesByIdsAsync(IEnumerable<Guid>)`. Resolve once per Detail render, pass into `MovementActivityMapper.Map(StockMovement, IReadOnlyDictionary<Guid,string> users, IReadOnlyDictionary<Guid,string> locations)`. Title becomes `"{user} received stock"`; description grows `"… at {location}"` / `"… from {from} to {to}"` for putaway pairs | Phase 6C ships title="Stock received" without actor + description="+5 units · ReferenceType" without location codes. Functional but anonymous. Acceptable for the demo; a polish task before any real audit-trail use |

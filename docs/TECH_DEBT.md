...
| TD-028 | Filter chip counts on list pages | Low | 2026-05-09 | Add count aggregates to the `/Data` JSON endpoint via single `SUM(CASE WHEN Status=...)` query alongside the existing rows + total. Frontend renders count next to each chip label (`All 24 / Open 12 / Receiving 8 / Closed 4 / Cancelled 0`). Same shape as Phase 8.5 picker chips. ~30 min per list page. Affects /PurchaseOrders + /Receiving (and any future list pages). Phase 10 polish target | Cosmetic. Picker page renders counts because the entire dataset is rendered server-side; list pages use client-side Alpine filter against paged JSON which has no count metadata. Functional path unaffected — chips still filter correctly without counts |
